...
class TelaAdministrador:
    def __init__(self):
        ...
        self.button_atividades_adm = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: TelatividadesADM(self.window_tela_adm),  # instancia o popup
            relief="flat"
            )
        ...
        self.button_bolsistas_adm = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: (self.window_tela_adm.destroy(), TelaCadUsuarios()),
            relief="flat"
        )
...
class TelatividadesADM:
    def __init__(self, parent):  # recebe a janela principal como argumento
        self.popup = Toplevel(parent)
        self.popup.geometry("770x640")
        self.popup.configure(bg="#FFFFFF")
        self.popup.resizable(False, False)
        self.popup.title("Nova Atividade")
        self.popup.transient(parent)  # torna modal (fica em cima da parent)
        self.popup.grab_set()         # bloqueia interação com janela principal
    ...
    #botões
        ...
        self.tela_atividade_button_2 = Button(
            ...
            borderwidth=0,
            highlightthickness=0,
            command=lambda: (self.popup.destroy(), TelaAdministrador()),
            relief="flat"
        )
...
class TelaCadUsuarios:
    def __init__(self):
        OUTPUT_PATH = Path(__file__).parent
        self.ASSETS_PATH = OUTPUT_PATH / "telas_adm" / "tela_cadbolsista" / "build" / "assets" / "frame0"
        self.windowtelacadbolsista = Tk()
        self.windowtelacadbolsista.resizable(False, False)
        self.windowtelacadbolsista.geometry("1440x846")
        self.windowtelacadbolsista.configure(bg = "#FFFFFF")
        self.canvastelacadbolsista = Canvas(
            self.windowtelacadbolsista,
            bg = "#FFFFFF",
            height = 846,
            width = 1440,
            bd = 0,
        ...
        self.canvastelacadbolsista.place(x = 0, y = 0)
        #botão1 = cadastrar bolsista
        self.image_image_1 = PhotoImage(
            file=self.relative_to_assets("image_1.png"))
        self.image_1 = self.canvastelacadbolsista.create_image(
            720.0,
            41.0,
        ...
        self.canvastelacadbolsista.create_text(
            246.0,
            160.0,
            anchor="nw",
        ...
        self.image_image_2 = PhotoImage(
            file=self.relative_to_assets("image_2.png"))
        self.image_2 = self.canvastelacadbolsista.create_image(
            350.0,
            242.0,
            image=self.image_image_2
        )
        #entry1 = campo para aparecer os bolsistas do filtro
        #temos que modificar isso e criar um campo que se autopreenche de acordo com a combobox
        self.entry_image_1 = PhotoImage(
            file=self.relative_to_assets("entry_1.png"))
        self.entry_bg_1 = self.canvastelacadbolsista.create_image(
            620.5,
            306.5,
        ...
        self.image_3 = self.canvastelacadbolsista.create_image(
            798.0,
            42.0,
        ...
        self.windowtelacadbolsista.mainloop()
    def relative_to_assets(self, path: str) -> Path:
        return self.ASSETS_PATH / Path(path)
        

class TelaCadUsuario2:
    
    def cadastrar_bolsista(self):
        conexao = conectar()
        cursor = conexao.cursor()
        #self.nome_usuario = nome_usuario.get()
        #self.nome_inst_usuario = nome_inst_usuario.get()
        #self.email_usuario = email_usuario.get()
        #self.senha_usuario = senha_usuario.get()
        #self.tipo_usuario = tipo_usuario.get()
        cursor.execute("INSERT INTO usuarios (nome, email, senha, tipo, nomeinst) VALUES (%s, %s, %s, %s, %s)", (self.nome_usuario, self.email_suario, self.senha_usuario, self.tipo_usuario, self.nome_inst_usuario))
        conexao.commit()
